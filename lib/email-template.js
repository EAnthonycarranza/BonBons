...
import { ORDER_STATUSES, QUOTE_STATUSES, CASH_AT_PICKUP, isPaidStatus } from "./order-tracking.js";
import { paymentChoiceUrl } from "./order-links.js";
...
// Email clients will not draw inline SVG, so the payment marks travel as
// attached PNGs like the logo does. Sizes match assets/email-payment-sizes.json.
export const EMAIL_PAYMENT_CIDS = {
  venmo: "bonbons-venmo@codingcarranza.com",
  cashapp: "bonbons-cashapp@codingcarranza.com",
  zelle: "bonbons-zelle@codingcarranza.com",
};

export const EMAIL_PAYMENT_MARKS = [
  { key: "venmo", label: "Venmo", width: 84, height: 28 },
  { key: "cashapp", label: "Cash App", width: 28, height: 28 },
  { key: "zelle", label: "Zelle", width: 28, height: 28 },
];
...
function details(record, kind) {
  ...
  return [
    ["Event type", record.occasion], ["Party size", partySizeLabel(record.guests)],
    ["Cake pops", record.interests?.join(", ")], ["Theme / colors", record.colors],
...
export function buildOrderEmail({ record, kind, emailType, personalMessage = "", delayNotice = false }) {
  ...
  // The customer told us they'll bring cash — at checkout or via the link in an
  // earlier email. It's intent, not payment, so the paid branch still wins.
  const cashChosen = !paid && record.paymentStatus === CASH_AT_PICKUP;
  const payment = isReceipt
    ? (cashChosen
        ? "You chose to pay cash at pickup. No payment is due until the owner confirms your order and total."
    ...
    : paid ? "Payment is recorded. Thank you!"
      : cashChosen ? SITE.paymentCashInstructions
  ...
  // One-click switches. Empty when no ADMIN_SECRET is set, in which case the
  // buttons are simply left out rather than pointing at a link that can't verify.
  const orderId = record.id || record._id;
  const cashUrl = canPay && !cashChosen ? paymentChoiceUrl(orderId, "cash") : "";
  const onlineUrl = canPay && cashChosen ? paymentChoiceUrl(orderId, "online") : "";

  // The marks keep their word alongside them: Outlook blocks images by default,
  // and a customer who sees nothing but "include order 1042" has lost the part
  // that tells them how to pay.
  const paymentMarksRow = `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:14px 0 0;"><tr>${EMAIL_PAYMENT_MARKS.map((mark, index) => `<td style="padding:0 ${index === EMAIL_PAYMENT_MARKS.length - 1 ? "0" : "8px"} 0 0;"><img src="cid:${EMAIL_PAYMENT_CIDS[mark.key]}" width="${mark.width}" height="${mark.height}" alt="${mark.label}" style="display:block;width:${mark.width}px;height:${mark.height}px;border:0;outline:none;text-decoration:none;"></td>`).join("")}</tr></table>`;
...
<body style="margin:0;padding:0;width:100%;background:#100b16;font-family:Arial,Helvetica,sans-serif;color:#2b2032;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#1b1025;border-radius:16px;border-collapse:separate;border-spacing:0;">
        ...
        <tr><td class="email-content" bgcolor="#fffaf5" style="padding:34px 36px 30px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#f0eaf2"><tr><td style="padding:16px 18px;border-left:3px solid #a882b7;">
            <p style="${labelStyle}margin:0 0 6px;">${isReceipt ? "No payment needed yet" : paid ? "Payment received" : cashChosen ? "Cash at pickup" : "Payment"}</p>
            <p style="margin:0;color:#594b60;font-size:13px;line-height:22px;">${escapeHtml(payment)}</p>
            ${canPay && !cashChosen ? `<p style="margin:15px 0 0;"><a href="${escapeHtml(SITE.paymentUrl)}" style="display:inline-block;padding:12px 20px;border-radius:5px;background:#b31665;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">Open payment options &rarr;</a></p>${paymentMarksRow}<p style="margin:10px 0 0;color:#594b60;font-size:12px;line-height:20px;">Put order <strong>${escapeHtml(record.orderNumber)}</strong> in the payment note. If you forget, reply to this email or text Bonnie with the app you paid from.</p><p style="margin:16px 0 0;padding-top:14px;border-top:1px solid #dfd3e3;color:#594b60;font-size:13px;line-height:21px;">Prefer to pay cash when you pick up? ${cashUrl ? `<a href="${escapeHtml(cashUrl)}" style="color:#a91963;font-weight:700;text-decoration:underline;">I\'ll pay cash at pickup &rarr;</a><br><span style="font-size:12px;color:#8a7d90;">Or simply reply to this email saying so.</span>` : `<strong style="color:#35273c;">Just reply to this email saying so</strong> and Bonnie will note it.`}</p>` : ""}${canPay && cashChosen ? `<p style="margin:15px 0 0;">${onlineUrl ? `<a href="${escapeHtml(onlineUrl)}" style="display:inline-block;padding:12px 20px;border-radius:5px;background:#b31665;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">Pay ahead online instead &rarr;</a>` : `<a href="${escapeHtml(SITE.paymentUrl)}" style="display:inline-block;padding:12px 20px;border-radius:5px;background:#b31665;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">Pay ahead online instead &rarr;</a>`}</p>${paymentMarksRow}` : ""}
          </td></tr></table>
  ...
  const text = [
    ...
    // The HTML shows the three marks as images; the plain-text copy has to say
    // them, for text-only clients and for anyone whose mail app blocks images.
    canPay && !cashChosen ? `Pay by Venmo, Cash App, or Zelle: ${SITE.paymentUrl}\nPut order ${record.orderNumber} in the payment note. If you forget, reply to this email or text Bonnie with the app you paid from.${cashUrl ? `\nPrefer cash at pickup? Use this link: ${cashUrl}\n(or just reply to this email saying so)` : "\nPrefer cash at pickup? Just reply to this email saying so."}` : "",
    canPay && cashChosen ? `Paying cash at pickup. Changed your mind? Pay ahead online: ${onlineUrl || SITE.paymentUrl}` : "",
