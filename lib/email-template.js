...
<body style="margin:0;padding:0;width:100%;background:#100b16;font-family:Arial,Helvetica,sans-serif;color:#2b2032;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#1b1025;border-radius:16px;border-collapse:separate;border-spacing:0;">
        ...
        <tr><td class="email-content" bgcolor="#fffaf5" style="padding:34px 36px 30px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#f0eaf2"><tr><td style="padding:16px 18px;border-left:3px solid #a882b7;">
            ...
            ${canPay ? `<p style="margin:15px 0 0;"><a href="${escapeHtml(SITE.paymentUrl)}" style="display:inline-block;padding:12px 20px;border-radius:5px;background:#b31665;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">Open payment options &rarr;</a></p>${paymentMarksRow}<p style="margin:10px 0 0;color:#594b60;font-size:12px;line-height:20px;">Include order ${escapeHtml(record.orderNumber)} with your payment, then confirm with Bonnie.</p>` : ""}
          </td></tr></table>
  ...
  const text = [
    ...
    canPay ? `Pay by Venmo, Cash App, or Zelle: ${SITE.paymentUrl}\nInclude order ${record.orderNumber} with your payment, then confirm with Bonnie.` : "",
