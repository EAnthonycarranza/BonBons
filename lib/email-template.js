...
export function buildOrderEmail({ record, kind, emailType, personalMessage = "" }) {
  if (!["request_received", "confirmation", "status_update"].includes(emailType)) throw new Error("Invalid email type.");
  ...
  if (!isReceipt && !record.orderNumber) throw new Error("An order number is required before sending a confirmation or update.");
  ...
  const headline = isReceipt ? "Your request is in."
    : isConfirmation ? "Your order is confirmed." : "An update on your order.";
  const intro = isReceipt
    ...
    : isConfirmation
      ? "Your cake-pop order is confirmed. Here's everything you need for pickup. Keep your order number handy if you have any questions."
  ...
  const preheader = isReceipt ? "Your request is saved. We'll be in touch to confirm the details. No payment is due yet." : `${record.orderNumber} · ${status} · Pickup details inside.`;
    ...
    <tr><td class="email-outer" align="center" style="padding:32px 12px;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#1b1025;border-radius:16px;border-collapse:separate;border-spacing:0;">
        ...
        <tr><td class="email-content" bgcolor="#fffaf5" style="padding:34px 36px 30px;">
          <p style="margin:0 0 12px;font-size:11px;line-height:17px;letter-spacing:1.5px;font-weight:700;color:#b31665;text-transform:uppercase;">${isReceipt ? "Pickup request received" : isConfirmation ? "Order confirmation" : "Order update"}</p>
