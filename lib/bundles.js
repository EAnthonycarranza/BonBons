...
/**
 * The Box of the Week is a cake-pop box. Pretzel rods bundle in twos on their
 * own shelf and are priced separately, so they must never end up inside it.
 *
 * Checked against the live menu rather than the submitted names, so renaming a
 * flavor or editing the request cannot slip a rod past this.
 */
export function assertBoxItemsAreCakePops(items = [], products = []) {
  const menu = new Map(products.map((product) => [product.slug, product]));
  for (const item of items) {
    const product = menu.get(String(item?.slug || ""));
    if (!product) {
      throw new Error(`${item?.name || "That item"} is no longer on the menu. Choose each item from the cake-pop menu.`);
    }
    if (bundleGroupForCategory(product.category) !== "cakepop") {
      throw new Error(`${product.name} is a pretzel rod, so it cannot go in the Box of the Week. The box holds cake pops only.`);
    }
  }
}
