...
export function documentLines(record, kind) {
  ...
  const detail = [
    record.occasion ? `Occasion: ${record.occasion}` : "",
    record.guests ? `Requested quantity: ${record.guests}` : "",
    record.interests?.length ? `Cake pops: ${record.interests.join(", ")}` : "",
    record.colors ? `Colors / theme: ${record.colors}` : "",
  ].filter(Boolean).join("  ·  ");
  return [{ name: "Custom cake-pop order", detail, qty: 1, amount: Number(record.confirmedTotal) || 0 }];
}
